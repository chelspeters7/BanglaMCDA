...
# Purpose: Complete a Generalized Sensitivity Analysis via a monte 
# carlo simulation for the MAVT MCDA.
...
for (kjl in 0:n+1){
  ...
  source("MAVT.r")
  rank.local[,kjl] <- local_norm
  rank.ngo[,kjl] <- ngo_norm
  rank.aca[,kjl] <- aca_norm
}
...
# ===========================================================================================================
# Plotting of Results
png(file = "MonteCarloRankingMAVT.png", width = 1152, height = 640)
...
# ===========================================================================================================
scorevalue <- 0.05#0.02
weightvalue <- 0.05

#LOCAL
# ===========================================================================================================
# Weighting distributions
png(file = "MAVT_Local_Weights.png", width = 1152, height = 640)
  beh.x <- beh.local.wei
  nbeh.x <- nbeh.local.wei
  source("DistributionPlot.r")
  mtext("MAVT, Local Perception, Top Two Ranks, Weights", outer = TRUE, cex = 1.5)
dev.off()
# ===========================================================================================================
...
png(file = "MAVT_Local_Scoring.png", width = 1152, height = 640)
  beh.x <- beh.local.sco
  nbeh.x <-nbeh.local.sco
  source("DistributionPlotScores.r")
  mtext("MAVT, Local Perception, Top Two Ranks, Scoring", outer = TRUE, cex = 1.5)
dev.off()
# ===========================================================================================================
...
png(file = "MAVT_NGO_Weights.png", width = 1152, height = 640)
  beh.x <- beh.ngo.wei
  nbeh.x <- nbeh.ngo.wei
  source("DistributionPlot.r")
  mtext("MAVT, NGO Perception, Top Two Ranks, Weights", outer = TRUE, cex = 1.5)
dev.off()
# ===========================================================================================================
...
png(file = "MAVT_NGO_Scoring.png", width = 1152, height = 640)
  beh.x <- beh.ngo.sco
  nbeh.x <-nbeh.ngo.sco
  source("DistributionPlotScores.r")
  mtext("MAVT, NGO Perception, Top Two Ranks, Scoring", outer = TRUE, cex = 1.5)
dev.off()
# ===========================================================================================================
...
png(file = "MAVT_ACA_Weights.png", width = 1152, height = 640)
  beh.x <- beh.aca.wei
  nbeh.x <- nbeh.aca.wei
  source("DistributionPlot.r")
  mtext("MAVT, Academic Perception, Top Two Ranks, Weights", outer = TRUE, cex = 1.5)
dev.off()
# ===========================================================================================================
# Testing weighting distributions
...
# ===========================================================================================================
# Scoring distributions
png(file = "MAVT_ACA_Scoring.png", width = 1152, height = 640)
  beh.x <- beh.aca.sco
  nbeh.x <-nbeh.aca.sco
  source("DistributionPlotScores.r")
  mtext("MAVT, Academic Perception, Top Two Ranks, Scoring", outer = TRUE, cex = 1.5)
dev.off()
# ===========================================================================================================
# Testing scoring distributions
...
png(file = "SignificantParameterDistributionsMAVT.png", width = 1152, height = 640)
plot <- ggplot(data2, aes(x = Behavior, y = Value, color = Stakeholder)) + 
  facet_wrap(~Label, ncol = 4)+
    geom_boxplot()+
  labs(title ="Significant Parameter Distribution in MAVT", x = "", y = "Score") +
  theme_classic() +
  theme_bw()+
...
png(file = "SignificantParameterDistributionsMAVTWeights.png", width = 1152, height = 640)
plot <- ggplot(dataw, aes(x = Behavior, y = Value, color = Stakeholder)) + 
  facet_wrap(~Criteria, ncol = 4)+
  geom_boxplot()+
  labs(title ="Significant Parameter Distribution in MAVT", x = "", y = "Weight") +
  theme_classic() +
  theme_bw()+
  theme(axis.text =  element_text(color = "black", size = 16))+
  theme(text = element_text(size = 30, family = "Garamond", color = "black"))+
